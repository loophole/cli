...
func forward(remoteEndpointSpecs lm.RemoteEndpointSpecs,
	...
	go func() {
		...
		if err != nil {
			communication.TunnelError(remoteEndpointSpecs.TunnelID, fmt.Sprintf("TLS Certificate failed to provision. Will be obtained with first request made by any client, therefore first execution may be slower: %v", err))
		} else {
